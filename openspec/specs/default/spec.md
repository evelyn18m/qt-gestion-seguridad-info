...
# Requirements from: update-valoracion-front-and-back

## ADDED Requirements

### Requirement: valTipoActivo ref declaration

The `valoracion.vue` page MUST declare `valTipoActivo` as a `ref([])` reactive reference near other `val*` refs. The ref MUST be initialized before the "Tipo de Active" dropdown change handler executes to prevent runtime crashes.
