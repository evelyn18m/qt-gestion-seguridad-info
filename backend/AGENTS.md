...
## Config quirks
- **Prisma adapter**: Seed and app use `@prisma/adapter-mariadb` explicitly (not default client). New PrismaClient calls must pass `{ adapter }`.
- **ESLint**: `no-explicit-any` is **off**, `no-floating-promises` and `no-unsafe-argument` are **warn**; `sourceType: 'commonjs'` in config
- **Jest config**: Defined inline in `package.json` (not a separate file), `rootDir: "src"` for unit tests, E2E config in `test/jest-e2e.json`
...
## New NestJS modules
Generate via `nest g resource <name>` (or `npx @nestjs/cli g resource <name>`). Add generated module to `imports` in `app.module.ts`.
