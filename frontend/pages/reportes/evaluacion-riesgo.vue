...
import type { EvaluacionRiesgoReporte, CatalogoItem } from '~/types/api'

definePageMeta({ layout: 'default' })

const loading = ref(false)
const errorMsg = ref('')
const rows = ref<EvaluacionRiesgoReporte[]>([])
const q = ref('')
...
const selectedNivelRiesgo = ref<string>('')
...
async function fetchData() {
  ...
  if (selectedNivelRiesgo.value) params.append('nivelRiesgo', selectedNivelRiesgo.value)
  const qs = params.toString()
  const path = `/reportes/evaluacion-riesgo${qs ? '?' + qs : ''}`
  try {
    loading.value = true
    errorMsg.value = ''
    rows.value = await apiFetch<EvaluacionRiesgoReporte[]>(path)
  } catch (e: unknown) {
    errorMsg.value = e instanceof Error ? e.message : 'Error al cargar evaluación de riesgo'
  } finally {
    loading.value = false
  }
}
...
async function exportExcel() {
  ...
  if (selectedNivelRiesgo.value) params.append('nivelRiesgo', selectedNivelRiesgo.value)
  const qs = params.toString()
  const path = `/reportes/evaluacion-riesgo/export${qs ? '?' + qs : ''}`
  ...
  try {
    ...
    a.download = `evaluacion-riesgo-${date}.xlsx`
    ...
    <div class="reportes-page">
      ...
      <aside class="filters-sidebar">
        <div class="sidebar-header">
          <h3>Filtros</h3>
          <button
            v-if="q || selectedMacroProceso || selectedCategoriaAmenaza || selectedAmenaza || selectedCategoriaVulnerabilidad || selectedVulnerabilidad || selectedNivelRiesgo"
            class="btn-clear"
            @click="clearFilters"
          >
            Limpiar
        ...
        <div class="filter-group">
          ...
          <select v-model="selectedNivelRiesgo" class="filter-select">
            <option value="">Todos</option>
      ...
      <main class="reportes-main">
        <div class="main-header">
          <div>
            ...
            <p class="subtitle">Vista consolidada con amenazas, vulnerabilidades, niveles e impacto.</p>
          </div>
        ...
        <template v-else>
          <div v-if="rows.length === 0" class="reportes-empty">
            No hay registros de evaluación de riesgo.
          </div>
            ...
            <table class="reportes-table">
              ...
              <tbody>
                <tr v-for="r in rows" :key="r.id">
                  ...
                  <td>{{ r.evaluacionRiesgo != null ? r.evaluacionRiesgo.toFixed(2) : '—' }}</td>
                  <td>
                    <span v-if="r.nivelRiesgo" :class="['nivel-badge', nivelClass(r.nivelRiesgo)]">
                      {{ r.nivelRiesgo }}
                    </span>
                    <span v-else>—</span>
                  </td>
                  <td>{{ r.controlesArea || '—' }}</td>
                </tr>
              </tbody>
            </table>
          </div>
